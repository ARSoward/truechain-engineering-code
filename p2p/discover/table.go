...
func (tab *Table) lookup(targetID NodeID, refreshIfEmpty bool) []*Node {
	...
	for {
		...
		log.Info("lookup", "result", result, "target", target, "targetID", targetID)
		tab.mutex.Unlock()
	...
	for {
		...
		for i := 0; i < len(result.entries) && pendingQueries < alpha; i++ {
			...
			if !asked[n.ID] {
				asked[n.ID] = true
				pendingQueries++
				log.Info("lookup", "n", n, "target", target, "targetID", targetID)
...
func (tab *Table) findnode(n *Node, targetID NodeID, reply chan<- []*Node) {
	...
	log.Info("findnode", "n", n, "targetID", targetID, "r", len(r))
	reply <- r
}
